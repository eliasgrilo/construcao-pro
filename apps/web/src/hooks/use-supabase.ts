...
export function useUpdateCategoria() {
    ...
    return useMutation({
        mutationFn: async ({ id, ...body }: { id: string; nome?: string; unidade?: string }) => {
            ...
            const { data, error } = await supabase.from('categorias').update(update as any).eq('id', id).select().single()
            if (error) throw error
            return data
        },
        onSuccess: () => {
...
export function useCreateMaterial() {
    const qc = useQueryClient()
    return useMutation({
        mutationFn: async (body: { nome: string; codigo: string; categoria_id: string; estoque_minimo?: number; codigo_barras?: string }) => {
...
export function useAprovarTransferencia() {
    ...
    return useMutation({
        mutationFn: async (movimentacaoId: string) => {
            const { data, error } = await supabase.rpc('aprovar_transferencia', { p_movimentacao_id: movimentacaoId })
            if (error) throw error
            return data
        },
        onSuccess: () => {
...
export function useCreateFornecedor() {
    const qc = useQueryClient()
    return useMutation({
        mutationFn: async (body: { nome: string; cnpj?: string; telefone?: string; email?: string; endereco?: string; observacao?: string }) => {

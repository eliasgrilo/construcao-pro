...
export function DashboardPage() {
  ...
  const obras = (custoPorObra || []).filter(
    (o: typeof custoPorObra extends (infer U)[] | undefined ? U : never) => o.status === 'ATIVA',
  )
  ...
  /* Terrenos em Standby */
  const terrenosStandby = (obrasData || []).filter(
    (o: typeof obrasData extends (infer U)[] | undefined ? U : never) => o.status === 'TERRENO',
  )
    ...
    <div className="pb-16">
            ...
            <div className="text-center md:text-left">
              ...
              <div className="flex justify-center md:justify-start gap-5 mt-4 pt-4 border-t border-border/20">
                {statusBreakdown.map((st) => {
                  const count =
                    obrasData?.filter(
                      (o: typeof obrasData extends (infer U)[] | undefined ? U : never) =>
                        o.status === st.key,
                    ).length ?? 0
                  return (
                    <motion.div
      ...
      >
        ...
        <div className="rounded-2xl bg-card p-4 md:p-5">
          ...
          >
            <AlertTriangle
              className="h-5 w-5"
              style={{ color: alertas.length > 0 ? clr.red : '#8E8E93' }}
            />
          </span>
      ...
      >
        <div className="flex items-baseline justify-between mb-4">
          ...
          <button
            onClick={() => navigate({ to: '/obras' })}
            className="text-[15px] md:text-[17px] text-primary font-regular flex items-center hover:text-primary/80 transition-colors"
          >
            Ver Todas
        ...
        {obras.length === 0 ? (
          ...
          >
            {obras.map((obra: typeof obras extends (infer U)[] ? U : never) => {
      ...
      >
        <div className="rounded-2xl bg-card overflow-hidden">
          ...
          {/* Items list */}
          {alertas.map((alerta: typeof alertas extends (infer U)[] ? U : never, i: number) => {
      ...
      >
        <div className="flex items-baseline justify-between mb-3">
          ...
          <button
            ...
            className="text-[15px] md:text-[17px] text-primary flex items-center hover:text-primary/80 transition-colors"
          >
            Ver Todas
            <ChevronRight className="h-4 w-4 ml-1" />
          </button>
        ...
        <div className="rounded-2xl bg-card overflow-hidden">
          {movs.length === 0 ? (
            <p className="text-[17px] text-muted-foreground text-center py-12">
              Nenhuma movimentação.
            </p>
          ) : (
            movs.map((mov: typeof movs extends (infer U)[] ? U : never, i: number) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[15px] md:text-[17px] font-medium truncate">
                      {mov.material?.nome ?? '—'}
                    </p>

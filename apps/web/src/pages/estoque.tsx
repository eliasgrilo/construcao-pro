  ...
  const { obraGroups, totals } = useMemo(() => {
    ...
    const totals = {
      ...
      cost: allItems.reduce(
        (s: number, i: any) => s + (i.quantidade ?? 0) * (i.material?.preco_unitario ?? 0),
        0,
      ),
      obras: groups.length,
      lowStock: alertCount,
      ...
      },
      {
        onSuccess: () => {
          toast({
            ...
            description: `${transfQty} ${transfTarget.unidade} de ${transfTarget.materialNome} transferidos.`,
            variant: 'success',
          })
                    ...
                    {obraGroups.map((group, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p className="text-[15px] font-semibold truncate pr-6">
                            {group.obraNome}
                          </p>
                          {group.obraEndereco && (
              ...
              {obraGroups.length === 0 && (
                <div className="text-center py-20">
                  ...
                  <p className="text-[15px] text-muted-foreground/70 mt-1 mb-6">
                    Adicione materiais ao estoque para começar
                  </p>
                  <Button onClick={() => setEntradaOpen(true)} size="sm">
                    <Plus className="h-4 w-4 mr-1.5" />
                    Nova Entrada
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Transfira material entre almoxarifados instantaneamente.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-3.5 rounded-xl bg-accent/50 px-4 py-3.5">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[14px] sm:text-[13px] font-semibold truncate">
                  {transfTarget?.materialNome}
                </p>
                <p className="text-[12px] text-muted-foreground">
                  De: {transfTarget?.almoxarifadoNome}
                </p>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="text-[18px] font-bold tabular-nums text-foreground">
                  {formatNumber(transfTarget?.quantidadeDisponivel ?? 0)}
                </p>
                <p className="text-[11px] text-muted-foreground">
                  {transfTarget?.unidade} disponível
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              {Number(transfQty.replace(',', '.')) > (transfTarget?.quantidadeDisponivel ?? 0) && (
                <p className="text-[12px] text-destructive flex items-center gap-1">
                  <AlertTriangle className="h-3 w-3" />
                  Quantidade excede o disponível (
                  {formatNumber(transfTarget?.quantidadeDisponivel ?? 0)} {transfTarget?.unidade})
                </p>
              )}
              {Number(transfQty.replace(',', '.')) > 0 &&
                Number(transfQty.replace(',', '.')) <=
                  (transfTarget?.quantidadeDisponivel ?? 0) && (
                  <p className="text-[12px] text-muted-foreground">
                    Restará:{' '}
                    <span className="font-semibold tabular-nums">
                      {formatNumber(
                    ...
                    {transfTarget?.unidade} na origem
                  </p>
                )}
            </div>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Registre a entrada de material em um almoxarifado.
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <strong>
                {formatNumber(deleteTarget?.quantidade ?? 0)} {deleteTarget?.unidade}
              </strong>{' '}
              de <strong>{deleteTarget?.materialNome}</strong> do almoxarifado{' '}
              <strong>{deleteTarget?.almoxarifadoNome}</strong>. Esta ação será registrada nas
              movimentações.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>

...
import { NovaContaReceberModal } from './nova-conta-receber-modal'
import { ReceberParcelaModal } from './receber-parcela-modal'
export function ContasAReceberSection({
  contas,
  obras,
}: {
  ...
  const handleReceberParcela = async (contaBancariaId: string, dataRecebimento: string, valor: number) => {
    ...
    try {
      await receberMutation.mutateAsync({
        parcelaId: receberModal.id,
        contaBancariaId,
        valor,
        dataRecebimento,
        descricao: desc,
        numeroParcela: receberModal.numero_parcela,
        totalParcelas: receberModal.total_parcelas,
      })
  ...
  const handleDeleteConta = (contaReceberId: string) => {
    setDeleteId(contaReceberId)
  }
  ...
  const confirmDelete = () => {
    ...
    deleteMutation.mutate(deleteId, {
      ...
      onError: () => {
        toast({ title: 'Erro ao excluir conta a receber', variant: 'error' })
        setDeleteId(null)
      },
    })
  }
  ...
  const totalRecebido = recebidas.reduce((s, p) => s + Number(p.valor), 0)
  const noPending = atrasadas.length === 0 && venceHoje.length === 0 && proximas.length === 0

  const renderParcelaRow = (
    p: ContaReceberParcela,
    opts: { urgency: 'atrasado' | 'hoje' | 'normal'; border?: boolean },
  ) => {
    const desc = p.contas_receber?.descricao ?? '—'
    const cliente = p.contas_receber?.cliente
    const isMulti = p.total_parcelas > 1
    const parsedDate = new Date(`${p.vencimento}T12:00:00`)
    const urgColor =
      opts.urgency === 'atrasado' ? clr.red : opts.urgency === 'hoje' ? clr.orange : clr.green
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[13.5px] font-medium line-clamp-2 leading-snug">{desc}</p>
        ...
        <div className="flex flex-col items-end gap-[5px] flex-shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <motion.button
              whileTap={{ scale: 0.9 }}
              onClick={() => handleDeleteConta(p.conta_receber_id)}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-[20px] md:text-[22px] font-bold tracking-tight">A Receber</h2>
        <motion.button
          whileTap={{ scale: 0.94 }}
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl bg-white dark:bg-white/[0.05] border border-border/15 dark:border-white/[0.06] overflow-hidden">
          <div className="flex flex-col items-center justify-center py-12 px-4 gap-2">
            ...
            >
              <ArrowDownRight className="h-7 w-7" style={{ color: '#34C759' }} strokeWidth={1.6} />
            </div>
            <p className="text-[15px] font-semibold mt-2">Nenhum recebimento</p>
            <p className="text-[13px] text-muted-foreground/55 text-center max-w-[220px] leading-relaxed">
              Cadastre valores a receber e acompanhe o cronograma de pagamentos
            </p>
            <motion.button
              whileTap={{ scale: 0.95 }}
              onClick={() => setNovaModalOpen(true)}
        ...
        <div className="space-y-3">
            ...
            >
              ...
              >
                ...
                <span className="text-[13px] font-bold tabular-nums" style={{ color: clr.red }}>
                  {formatCurrency(atrasadas.reduce((s, p) => s + Number(p.valor), 0))}
                </span>
              </div>
          ...
          {proximasMeses.map((key, mi) => {
            ...
            return (
              ...
              >
                <div
                  className="flex items-center justify-between px-4 md:px-5 py-[10px]"
                  style={{ backgroundColor: isFirst ? `${clr.green}07` : undefined }}
                >
                  <div className="flex items-center gap-2">
                    <span
                      className="text-[11.5px] font-bold uppercase tracking-[0.55px] capitalize"
                      style={{ color: isFirst ? clr.green : '#8E8E93' }}
                    >
                      {grupo.mesLabel}
                    </span>
                    {isFirst && (
                      <span
                        ...
                        style={{ backgroundColor: `${clr.green}15`, color: clr.green }}
                      >
                        Próximo
                      </span>
                    )}
          ...
          {/* Tudo em dia — no pending but has items */}
          {noPending && recebidas.length > 0 && (
            <div className="rounded-2xl bg-white dark:bg-white/[0.05] border border-border/15 dark:border-white/[0.06] overflow-hidden">
                  ...
                  >
                    <div className="border-t border-border/10 dark:border-white/[0.05]">
                      {recebidas.map((p, i) => {
                        ...
                        const cliente = p.contas_receber?.cliente
                        const isMulti = p.total_parcelas > 1
                        const dateStr = p.recebido_em ?? p.vencimento
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {cliente && (
                                <p className="text-[11px] text-muted-foreground/40 mt-[2px] truncate">
                                  {cliente}
                                </p>
                              )}
                              {isMulti && (
                            ...
                            <div className="flex flex-col items-end gap-[4px] flex-shrink-0">
                              ...
                              <motion.button
                                whileTap={{ scale: 0.9 }}
                                onClick={() => handleDeleteConta(p.conta_receber_id)}
                                className="flex h-5 w-5 items-center justify-center rounded-full opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity mt-0.5"
                                style={{ backgroundColor: '#FF3B3012' }}
                              >
                                <Trash2 className="h-2.5 w-2.5" style={{ color: clr.red }} />
                              </motion.button>
                            </div>
                          </div>
                        )
      ...
      <ReceberParcelaModal
        ...
        onConfirm={handleReceberParcela}
        isPending={receberMutation.isPending}
      />

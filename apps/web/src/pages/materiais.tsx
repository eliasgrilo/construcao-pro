...
import { Barcode, Package, Pencil, Plus, Tag, Trash2 } from 'lucide-react'
  ...
  const columns: ColumnDef<any>[] = [
    {
      ...
      header: 'Código',
      cell: ({ row }) => (
        <span className="font-mono text-[11px] text-muted-foreground">{row.original.codigo}</span>
      ),
    },
    {
      accessorKey: 'nome',
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-muted-foreground">
            {row.original.categoria?.unidade || ''}
          </span>
        </div>
      ),
    },
    {
      ...
      header: 'Estoque Mín.',
      cell: ({ row }) => (
        <span className="tabular-nums text-[13px]">{row.original.estoque_minimo ?? 0}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Button
          variant="ghost"
          size="icon"
          onClick={() => setDeleteTarget({ id: row.original.id, nome: row.original.nome })}
    ...
    <>
      ...
      <div className="px-4 md:px-8 mt-5 mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2.5 rounded-xl bg-accent/50 px-4 py-3 text-[13px] text-muted-foreground flex-1 mr-3">
          ...
          <span>
            Cadastre materiais aqui. Para <strong>entrada de estoque</strong> com preço e
            quantidade, vá em <strong>Estoque → Nova Entrada</strong>.
          </span>
        </div>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Cadastre o material no catálogo. Preço e quantidade são definidos na entrada de
              estoque.
            </DialogDescription>
          </DialogHeader>
          <form
          ...
          >
            ...
            <div className="space-y-2">
              ...
              {errors.nome && (
                <p className="text-[13px] text-destructive mt-1">{errors.nome.message}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {errors.codigo && (
                <p className="text-[13px] text-destructive mt-1">{errors.codigo.message}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {errors.categoriaId && (
                <p className="text-[13px] text-destructive mt-1">{errors.categoriaId.message}</p>
              )}
            </div>
            ...
            {selectedCategoria && (
              <div className="flex items-center gap-2 rounded-xl bg-accent/50 px-4 py-3">
                <span className="text-[14px] sm:text-[13px] text-muted-foreground">
                  Unidade de medida:
                </span>
                <span className="text-[14px] sm:text-[13px] font-semibold">
                  {selectedCategoria.unidade}
                </span>
              </div>
            )}
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Crie rapidamente uma categoria. Ela será selecionada automaticamente.
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="flex items-center gap-3 rounded-xl bg-accent/50 px-4 py-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-muted-foreground">
                    Unidade: {getUnidadeLabel(quickCatUnidade)} ({getUnidadeAbbr(quickCatUnidade)})
                  </p>
                </div>
              </div>
            )}
              ...
              <Button
                onClick={() =>
                  createCategoriaMutation.mutate(
                    { nome: quickCatNome.trim(), unidade: quickCatUnidade },
                    {
                      onSuccess: (data: any) => {
                        setQuickAddCatOpen(false)
                        setQuickCatNome('')
        ...
        {isLoading ? (
          <div className="space-y-3">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="rounded-xl border bg-card p-5 animate-pulse flex items-center gap-4"
              >
          ...
          <div className="text-center py-20">
            ...
            <p className="text-[15px] text-muted-foreground/70 mt-1 mb-6">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[15px] sm:text-[14px] font-semibold truncate">
                        {cat.nome}
                      </p>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Nova Categoria</DialogTitle>
            <DialogDescription>
              Crie uma categoria para organizar os materiais do catálogo.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-5">
            <div className="space-y-2">
            ...
            {newNome && newUnidade && (
              <div className="flex items-center gap-3 rounded-xl bg-accent/50 px-4 py-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-muted-foreground">
                    Unidade: {getUnidadeLabel(newUnidade)} ({getUnidadeAbbr(newUnidade)})
                  </p>
                </div>
              </div>
            )}
            ...
            <DialogFooter>
              ...
              <Button
                onClick={() =>
                  createMutation.mutate(
                    { nome: newNome.trim(), unidade: newUnidade },
                    {
                      onSuccess: () => {
                        setCreateOpen(false)
                        resetCreate()
      ...
      <Dialog open={!!editTarget} onOpenChange={(v) => !v && setEditTarget(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Altere o nome ou a unidade de medida da categoria.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-5">
            <div className="space-y-2">
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {deleteTarget?.count && deleteTarget.count > 0 ? (
                <>
                  Tem certeza que deseja excluir <strong>{deleteTarget?.nome}</strong>? Existem{' '}
                  <strong>{deleteTarget?.count} materiais</strong> vinculados a esta categoria. Essa

...
const statusAprovacao: Record<string, { label: string; variant: 'success' | 'destructive' | 'warning' }> = {
...
export function MovimentacoesPage() {
    ...
    const columns: ColumnDef<any>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => <span className="font-semibold tabular-nums text-[13px]">{formatNumber(row.original.quantidade)}</span>,
        },
        {
            accessorKey: 'preco_unitario', header: 'Valor Un.',
            cell: ({ row }) => {
                const p = row.original.preco_unitario
                return p ? <span className="text-[13px] tabular-nums">{formatCurrency(p)}</span> : <span className="text-muted-foreground">—</span>
            },
        },
        {
            ...
            cell: ({ row }) => {
                ...
                const cfg = tipoConfig[row.original.tipo] || tipoConfig.ENTRADA
                return (
                    <span className={`text-[13px] font-semibold tabular-nums ${row.original.tipo === 'SAIDA' ? 'text-destructive' : 'text-green-600 dark:text-green-400'}`}>
                        {row.original.tipo === 'SAIDA' ? '−' : '+'}{formatCurrency(total)}
                    </span>
                )
            },
        },
        {
            ...
            cell: ({ row }) => (
                <div>
                    ...
                    <p className="text-[11px] text-muted-foreground">{row.original.almoxarifado?.obra?.nome || ''}</p>
                </div>
            ),
        },
        {
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                const labels: Record<string, string> = {
                    ...
                    BOLETO: 'Boleto', TRANSFERENCIA: 'Transf.', DINHEIRO: 'Dinheiro', CHEQUE: 'Cheque',
                }
                return <span className="text-[13px]">{labels[fp] || fp}</span>
            },
        },
        {
            accessorKey: 'usuario.nome', header: 'Usuário',
            cell: ({ row }) => <span className="text-[13px] text-muted-foreground">{row.original.usuario?.nome || '—'}</span>,
        },
        {
            accessorKey: 'created_at', header: 'Data',
            cell: ({ row }) => <span className="text-[13px] text-muted-foreground tabular-nums">{formatDate(row.original.created_at)}</span>,
        },
    ]
    ...
    return (
        <div className="pb-10">
            <div className="px-4 md:px-8 pt-10 pb-6">
                ...
                <p className="text-[13px] text-muted-foreground mt-0.5">Histórico de entradas, saídas e transferências</p>
            </div>
            ...
            <div className="px-4 md:px-8 mb-5">
                <div className="flex items-center gap-2.5 rounded-xl bg-accent/50 px-4 py-3 text-[13px] text-muted-foreground">
                    <span>📦</span>
                    <span>As movimentações são registradas automaticamente. Para dar <strong>entrada de material</strong>, vá para <strong>Estoque → Nova Entrada</strong>.</span>
                </div>
            </div>

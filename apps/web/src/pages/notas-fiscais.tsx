    ...
    const columns: ColumnDef<any>[] = [
        {
            ...
            cell: ({ row }) => (
                <div className="flex items-center gap-2">
                    <div className="flex h-7 w-7 items-center justify-center rounded-lg" style={{ backgroundColor: '#007AFF14' }}>
        ...
        {
            accessorKey: 'cnpj_emitente', header: 'CNPJ',
            cell: ({ row }) => <span className="font-mono text-[11px] text-muted-foreground">{row.original.cnpj_emitente}</span>,
        },
        {
            accessorKey: 'valor_total', header: 'Valor',
            cell: ({ row }) => <span className="font-semibold text-[13px] tabular-nums">{formatCurrency(row.original.valor_total)}</span>,
        },
        {
            accessorKey: 'status', header: 'Status',
            cell: ({ row }) => {
                const s = statusMap[row.original.status]
                return s ? <Badge variant={s.variant}>{s.label}</Badge> : <Badge variant="secondary">{row.original.status}</Badge>
            },
        },
        {
            accessorKey: '_count.itens', header: 'Itens',
            cell: ({ row }) => <span className="tabular-nums text-[13px]">{row.original._count?.itens || 0}</span>,
        },
        {
            accessorKey: 'data_emissao', header: 'Emissão',
            cell: ({ row }) => <span className="text-[11px] text-muted-foreground tabular-nums">{formatDate(row.original.data_emissao)}</span>,
        },
    ]
    ...
    return (
        <div className="pb-10">
            <div className="px-4 md:px-8 pt-10 pb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                ...
                <Button onClick={() => setOpen(true)} size="sm"><Upload className="h-4 w-4 mr-1.5" />Upload XML</Button>
            </div>
            <div className="px-4 md:px-8">
                <DataTable columns={columns} data={nfsData || []} isLoading={isLoading} searchPlaceholder="Buscar notas fiscais..." />
            </div>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>Selecione ou arraste o arquivo XML da nota fiscal.</DialogDescription>
                    </DialogHeader>
                        ...
                        >
                            {file ? (
                                <>
                                    ...
                                    <p className="text-[13px] sm:text-[11px] text-muted-foreground mt-1">{(file.size / 1024).toFixed(1)} KB</p>
                                </>
                            ) : (
                                <>
                                    <CloudUpload className="h-10 w-10 sm:h-8 sm:w-8 text-muted-foreground mx-auto mb-3" />
                                    <p className="text-[15px] sm:text-[13px] font-medium">Arraste o XML ou toque para selecionar</p>
                                    <p className="text-[13px] sm:text-[11px] text-muted-foreground mt-1">Formato aceito: .xml</p>
                                </>
                            )}
                            <input id="xml-input" type="file" accept=".xml" className="hidden" onChange={(e) => setFile(e.target.files?.[0] || null)} />
                        </div>
                        <DialogFooter>
                            <Button variant="outline" onClick={() => { setOpen(false); setFile(null) }}>Cancelar</Button>
                            <Button onClick={handleUpload} disabled={!file} loading={uploading}>Processar XML</Button>
                        </DialogFooter>
                    </div>
                </DialogContent>
            </Dialog>

...
export function ObraDetailPage() {
  ...
  const movAlmoxEstoqueItems = useMemo(() => {
    ...
    return (estoque as any[]).filter(
      (e: any) => e.almoxarifado?.id === movAlmoxId && (e.quantidade ?? 0) > 0,
    )
  }, [movAlmoxId, estoque])
    ...
    <div className="pb-10">
      ...
      <div className="px-4 md:px-8 pt-6 pb-0">
        <button
          ...
          className="flex items-center gap-1 text-[13px] text-primary hover:text-primary/80 transition-colors mb-5"
        >
        ...
        <div className="grid grid-cols-2 gap-4 md:flex md:gap-10 mt-7 pb-6 border-b">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              Orçamento{' '}
              <span className="text-primary/60 hidden sm:inline">· clique para editar</span>
            </p>
          </div>
          {/* Other metrics */}
          {[
            ...
            <div key={m.label}>
              <p className="text-[20px] md:text-[26px] font-semibold tabular-nums leading-none">
                {m.value}
              </p>
      ...
      >
                  ...
                  <div className="space-y-2.5 flex-1 min-w-0">
                    ...
                    <div className="flex justify-between items-center text-[13px]">
                      ...
                      {editingTerreno ? (
                        ...
                        <span
                          ...
                          onClick={() => {
                            setTerrenoInput(String(custos.valorTerreno ?? 0))
                    ...
                    {obra?.status === 'VENDIDO' && (custos.valorVenda ?? 0) > 0 ? (
                      (() => {
                        ...
                        const margem =
                          (custos.total ?? 0) > 0
                            ? ((lucro / (custos.total ?? 0)) * 100).toFixed(1)
                            : '0.0'
                        return (
                      ...
                      <div className="border-t border-border/30 pt-2.5">
                        <div className="flex justify-between text-[13px]">
                          ...
                          <span
                            className={cn(
                              'font-semibold tabular-nums',
                              (custos.saldo ?? 0) < 0 ? 'text-destructive' : 'text-success',
                            )}
                          >
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 space-y-2.5 ml-2">
                      {(custos.porCategoria || []).map((cat: any, i: number) => {
                        ...
                        const totalPie = (custos.porCategoria || []).reduce(
                      ...
                      <li key={tx.id} className="flex items-center gap-3.5 px-5 py-3.5">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            {tx.quantidade} ×{' '}
                            {formatCurrency(tx.preco_unitario ?? tx.precoUnitario)} ·{' '}
                            {tx.almoxarifado}
                          </p>
                        </div>
                ...
                <div className="rounded-xl border bg-card overflow-hidden">
                      ...
                      <li key={item.id} className="flex items-center gap-3.5 px-5 py-3.5">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            ...
                            {formatCurrency(item.preco_unitario ?? item.precoUnitario)} ·{' '}
                            {item.almoxarifado}
                          </p>
                        </div>
                  ...
                  <div className="flex justify-between items-center px-5 py-3 border-t bg-muted/30">
                    ...
                    <span className="text-[15px] font-bold tabular-nums">
                      {formatCurrency(custos.realizado)}
                    </span>
                  </div>
                </div>
              )}
        ...
        {activeTab === 'almoxarifados' && (
          <div>
            ...
            {almoxarifados.length === 0 ? (
              <Empty
                icon={Warehouse}
                text="Nenhum almoxarifado"
                sub="Adicione um almoxarifado para gerenciar o estoque desta obra."
              />
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {almoxarifados.map((a: any) => {
                  const aEstoque = estoque.filter((e: any) => e.almoxarifado?.id === a.id)
                  const totalQty = aEstoque.reduce(
                    (s: number, e: any) => s + (e.quantidade || 0),
                    0,
                  )
                  const lowStock = aEstoque.filter(
                    (e: any) =>
                      e.material?.estoque_minimo > 0 &&
                      (e.quantidade ?? 0) <= e.material.estoque_minimo,
                  )
                  return (
                    <div key={a.id} className="group rounded-xl border bg-card p-5">
                      <div className="flex items-start justify-between">
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-7 w-7 text-muted-foreground/40 hover:text-destructive opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity"
                          onClick={() => setDeleteTarget({ id: a.id, nome: a.nome })}
                        >
                          ...
                          {aEstoque.map((e: any) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span
                                  className={cn(
                                    'font-medium tabular-nums ml-2 flex items-center gap-1',
                      ...
                      <div className="flex items-center gap-4 mt-4 pt-3 border-t border-border/50">
                        ...
                        {lowStock.length > 0 && (
                          <div className="ml-auto flex items-center gap-1 text-[11px] text-warning font-medium">
        ...
        {activeTab === 'estoque' &&
          (() => {
            const lowCount = estoque.filter(
              (e: any) =>
                e.material?.estoque_minimo > 0 && (e.quantidade ?? 0) <= e.material.estoque_minimo,
            ).length
            ...
            return (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Button size="sm" onClick={() => setEntradaDialog(true)}>
                    <Plus className="h-4 w-4 mr-1.5" />
                    Entrada
                  </Button>
                </div>
                ...
                <div className="flex items-center gap-2.5 rounded-xl bg-accent/50 px-4 py-3 text-[13px] text-muted-foreground">
                  ...
                  <span>
                    Use <strong>Baixa</strong> para registrar uso de materiais. Use{' '}
                    <strong>↔</strong> para transferir entre almoxarifados.
                  </span>
                </div>
              ...
              <div className="rounded-xl border bg-card overflow-hidden">
                <ul className="divide-y divide-border/50">
                  {movimentacoes.map((mov: any) => {
                    ...
                    const Icon = t.icon
                    const st = mov.status_transferencia
                      ? statusTransf[mov.status_transferencia]
                      : null
                      ...
                      <li key={mov.id} className="flex items-center gap-3.5 px-5 py-3.5">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {st && (
                              <Badge variant={st.variant} className="text-[10px] py-0 h-[18px]">
                                {st.label}
                              </Badge>
                            )}
                          </div>
                          <p className="text-[11px] text-muted-foreground">
                            ...
                            {mov.tipo === 'TRANSFERENCIA' && mov.almoxarifado_destino
                              ? ` → ${mov.almoxarifado_destino.nome}`
                              : ''}
          ...
          <div className="space-y-5">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ex: Almoxarifado Bloco C"
              />
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setCreateAlmoxOpen(false)}>
                Cancelar
              </Button>
              <Button
                onClick={() =>
                  createAlmox.mutate(
                    { nome: newAlmoxName, obra_id: obraId },
                    {
                      onSuccess: () => {
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(v) => !v && setDeleteTarget(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Tem certeza que deseja excluir <strong>{deleteTarget?.nome}</strong>? Todo o estoque e
              histórico de movimentações deste almoxarifado serão excluídos permanentemente.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-3.5 rounded-xl bg-accent/50 px-4 py-3.5">
              ...
              <div className="text-right flex-shrink-0">
                ...
                <p className="text-[11px] text-muted-foreground">
                  {baixaTarget?.unidade} disponível
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              {Number(baixaQty.replace(',', '.')) > (baixaTarget?.quantidadeDisponivel ?? 0) && (
                <p className="text-[12px] text-destructive flex items-center gap-1">
                  ...
                  {formatNumber(baixaTarget?.quantidadeDisponivel ?? 0)} {baixaTarget?.unidade})
                </p>
              )}
              {Number(baixaQty.replace(',', '.')) > 0 &&
                Number(baixaQty.replace(',', '.')) <= (baixaTarget?.quantidadeDisponivel ?? 0) && (
                  <p className="text-[12px] text-muted-foreground">
                    Restará:{' '}
                    <span className="font-semibold tabular-nums">
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Informe o valor de venda desta obra para calcular o lucro.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-5">
            {/* Investment summary */}
                    ...
                    >
                      <div className="flex items-center justify-center gap-1.5 mb-1">
                        ...
                        <span
                          ...
                          style={{
                            color: isPositive ? 'var(--color-success)' : 'var(--color-destructive)',
                          }}
                        >
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setVendaDialog(false)}>
                Cancelar
              </Button>
              <Button
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Registre a entrada de material em um almoxarifado desta obra.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-5">
            {/* 1. Material */}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <CurrencyInput
                  ...
                  placeholder={
                    (selectedEntMaterial?.preco_unitario ?? 0) > 0
                      ? String(selectedEntMaterial!.preco_unitario).replace('.', ',')
                      : '0,00'
                  }
                />
              </div>
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={resetEntrada}>
                Cancelar
              </Button>
              <Button
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Registre uma entrada, saída ou transferência de material.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-5">
                ...
                {(['ENTRADA', 'SAIDA', 'TRANSFERENCIA'] as const).map((t) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        setMovTipo(t)
                        if (t !== 'TRANSFERENCIA') setMovAlmoxDestinoId('')
                      }}
                      className={cn(
                    ...
                    >
                      <Icon
                        ...
                        style={{ color: active ? cfg.tint : undefined }}
                      />
                      {cfg.label}
                    </button>
                  )
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => {
                  setMovAlmoxId(v)
                  setMovQty('')
                  if (movTipo === 'TRANSFERENCIA') setMovMaterialId('')
                }}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Selecione" />
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger>
                    <SelectValue
                      placeholder={
                        movAlmoxId ? 'Selecione o material' : 'Selecione o almoxarifado primeiro'
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
                    {movAlmoxEstoqueItems.length === 0 && movAlmoxId ? (
                      ...
                      movAlmoxEstoqueItems.map((item: any) => (
                        <SelectItem key={item.material?.id} value={item.material?.id}>
                          {item.material?.nome} — {formatNumber(item.quantidade)}{' '}
                          {item.material?.categoria?.unidade ?? 'UN'}
                        </SelectItem>
                      ))
                    )}
                  </SelectContent>
            ...
            {movTipo === 'TRANSFERENCIA' && movTransfDisponivel && (
              <div className="flex items-center justify-between rounded-xl bg-accent/50 px-4 py-3">
                ...
                <span className="text-[15px] font-bold tabular-nums">
                  {formatNumber(movTransfDisponivel.quantidade)}{' '}
                  <span className="text-[13px] font-normal text-muted-foreground">
                    {movTransfDisponivel.unidade}
                  </span>
                </span>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              {movTipo === 'TRANSFERENCIA' &&
                ...
                Number(movQty.replace(',', '.')) > movTransfDisponivel.quantidade && (
                  <p className="text-[12px] text-destructive flex items-center gap-1">
                    <AlertTriangle className="h-3 w-3" />
                    Excede o disponível ({formatNumber(movTransfDisponivel.quantidade)}{' '}
                    {movTransfDisponivel.unidade})
                  </p>
                )}
              {movTipo === 'TRANSFERENCIA' &&
                movTransfDisponivel &&
                Number(movQty.replace(',', '.')) > 0 &&
                Number(movQty.replace(',', '.')) <= movTransfDisponivel.quantidade && (
                  <p className="text-[12px] text-muted-foreground">
                    Restará:{' '}
                    <span className="font-semibold tabular-nums">
                      {formatNumber(
                        movTransfDisponivel.quantidade - Number(movQty.replace(',', '.')),
                      )}
                    </span>{' '}
                    {movTransfDisponivel.unidade}
                  </p>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={resetMovForm}>
                Cancelar
              </Button>
              <Button
      ...
      <Dialog
        open={!!deleteEstoqueTarget}
        onOpenChange={(v) => {
          if (!v) setDeleteEstoqueTarget(null)
        }}
      >
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <strong>
                {formatNumber(deleteEstoqueTarget?.quantidade ?? 0)} {deleteEstoqueTarget?.unidade}
              </strong>{' '}
              de <strong>{deleteEstoqueTarget?.materialNome}</strong> do almoxarifado{' '}
              <strong>{deleteEstoqueTarget?.almoxarifadoNome}</strong>. Esta ação será registrada
              nas movimentações.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteEstoqueTarget(null)}>
              Cancelar
            </Button>
            <Button
              variant="destructive"
              onClick={() =>
                deleteEstoqueTarget &&
                deleteEstoqueTarget.quantidade > 0 &&
                createSaida.mutate(
                  {
                  ...
                  },
                  {
                    onSuccess: () => {
                      toast({
                        ...
                        description: 'Saída registrada nas movimentações.',
                        variant: 'success',
                      })

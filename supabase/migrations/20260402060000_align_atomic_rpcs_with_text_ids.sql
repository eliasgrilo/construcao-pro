-- ─────────────────────────────────────────────────────────────────────────────
-- Align atomic RPC signatures with legacy TEXT ids
--
-- Core business tables in this project still use TEXT ids. The RPC layer
-- must accept TEXT for those references, only keeping UUID for tables that
-- are truly UUID-backed (financeiro_* and contas_*).
-- ─────────────────────────────────────────────────────────────────────────────

DROP FUNCTION IF EXISTS start_obra_manutencao(UUID, TEXT, TEXT[]);
DROP FUNCTION IF EXISTS update_usuario_with_obras(UUID, TEXT, TEXT, BOOLEAN, UUID[]);
DROP FUNCTION IF EXISTS create_entrada_estoque_financeiro(UUID, NUMERIC, NUMERIC, UUID, UUID, TEXT, TEXT, DATE, UUID, TEXT, TEXT, TEXT, UUID);
DROP FUNCTION IF EXISTS delete_nota_fiscal_orchestrated(UUID, BOOLEAN);
DROP FUNCTION IF EXISTS register_obra_sale(UUID, NUMERIC, JSONB, JSONB);
...
CREATE OR REPLACE FUNCTION update_usuario_with_obras(
  ...
  p_role       TEXT,
  p_ativo      BOOLEAN,
  p_obra_ids   TEXT[] DEFAULT ARRAY[]::TEXT[]
)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION update_usuario_with_obras(TEXT, TEXT, TEXT, BOOLEAN, TEXT[]) TO authenticated;
GRANT EXECUTE ON FUNCTION create_entrada_estoque_financeiro(TEXT, NUMERIC, NUMERIC, TEXT, UUID, TEXT, TEXT, DATE, TEXT, TEXT, TEXT, TEXT, TEXT) TO authenticated;
